...
public class IndexFiles {
  ...
  public static void main(String[] args) {
	...
	try {
		stream = Files.newInputStream(Paths.get(docspath));
  ...
  static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
    try (InputStream stream = Files.newInputStream(file)) {
  	...
  	List<List<String>> documents = Reuters21578Parser.parseString(buff);
  	
  	
  	for(List<String> sdoc : documents){
  	
  	
  	//añadir los campos del doc
  	
  		doc.add(new TextField("TOPICS", sdoc.get(1),Field.Store.YES));
